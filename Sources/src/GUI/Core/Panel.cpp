...
struct Panel::ImGuiNewFrameCallback : public osg::Camera::DrawCallback
{
    ImGuiNewFrameCallback(Panel& handler)
        : handler_(handler)
    {
    }
...
struct Panel::ImGuiRenderCallback : public osg::Camera::DrawCallback
{
    ImGuiRenderCallback(Panel& handler)
        : handler_(handler){
    }
...
void Panel::render(osg::RenderInfo&)
{
    drawUi();
    ImGui::Render();
...
bool Panel::handle(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa)
{
    if (!initialized_)
    {
        auto view = aa.asView();
